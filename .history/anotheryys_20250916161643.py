...
class YYSBotGUI:
    ...
    def button_choice(self, img_address1, img_address2, con: float = 0.8, fre: float = 0.1):
        """
        轮询检测两个图像，返回第一个被检测到的图像位置
        专门用于检测战斗结果（胜利或失败）
        """
        while self.is_running:
            self.pause_event.wait()  # 检查是否暂停
            try:
            ...
            for i in range(1, cycles + 1):
                ...
                while not end_pos and self.is_running:
                    ...
                    if explore_pos is not None:
                        ...
                        self.log("检测到战斗结果，点击确认")
                        time.sleep(2 + random.uniform(0, 1))

                        # 如果是胜利，检查是否有战利品结算
                        if self.victory_img_var.get() in str(result_pos) or \
                           pyautogui.locateCenterOnScreen(
                               self.victory_img_var.get(),
                               confidence=result_confidence) is not None:
                            reward_pos = self.find_image(
                                self.reward_img_var.get(), reward_confidence, timeout=5)
                            if reward_pos:
                                self.click_position(reward_pos)
                                self.log("检测到战利品结算，点击确认")
                                time.sleep(1.5)
        ...
        try:
            ...
            for i in range(1, cycles + 1):
                ...
                result_pos = self.button_choice(
                    self.victory_img_var.get(),
                    self.defeat_img_var.get(),
                    max(victory_confidence, defeat_confidence),  # 使用较高的置信度
                    0.1  # 高频检测
                )
