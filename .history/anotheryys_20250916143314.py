        ...
        try:
            ...
            for i in range(1, cycles + 1):
                ...
                while not result_pos and wait_count < max_wait and self.is_running:
                    ...
                    # 如果没有失败，再检测胜利/结算
                    if not defeat_pos and not reward_pos:
                        reward_pos = self.find_image(
                            self.reward_img_var.get(), result_confidence)

                    result_pos = defeat_pos or reward_pos
                    wait_count += 1
                    time.sleep(0.5)
                ...
                if result_pos:
                    ...
                    # 如果是胜利，检查是否还有额外结算
                    if not defeat_pos:  # defeat_pos为None表示不是失败而是胜利/结算
