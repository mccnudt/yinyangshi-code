...
class YYSBotGUI:
    ...
    def setup_ui(self):
        ...
        self.special_img_frame.grid(row=4, column=0, columnspan=4,
                                    sticky=(tk.W, tk.E), pady=(5, 0))

        # 参数设置
        param_frame = ttk.Frame(battle_frame)
        param_frame.grid(row=5, column=0, columnspan=4,
    ...
    def update_special_images(self):
        ...
        if template_name == "困难28":
            ...
            ttk.Entry(self.special_img_frame, textvariable=self.explore_img_var,
                      width=15).grid(row=0, column=4, padx=(5, 5))
            ttk.Button(self.special_img_frame, text="浏览", command=lambda: self.browse_image(
                "explore_img")).grid(row=0, column=5, padx=(0, 5))
            ...
            ttk.Entry(self.special_img_frame, textvariable=self.treasure_img_var,
                      width=15).grid(row=1, column=4, padx=(5, 5), pady=(5, 0))
            ttk.Button(self.special_img_frame, text="浏览", command=lambda: self.browse_image(
                "treasure_img")).grid(row=1, column=5, padx=(0, 5), pady=(5, 0))
            ...
            if template:
                ...
                if "kun28_img" in template:
                    if hasattr(self, 'kun28_img_var'):
                        self.kun28_img_var.set(template.get("kun28_img", ""))
    ...
    def kun28_strategy(self):
        ...
        try:
            ...
            for i in range(1, cycles + 1):
                ...
                # 查找困难28按钮
                kun28_pos = self.find_image(self.kun28_img_var.get() if hasattr(
                    self, 'kun28_img_var') else 'kun28.png', 0.8)
                start_pos = self.find_image(self.battle_img_var.get(), 0.7)
                ...
                # 探索过程
                end_pos = self.find_image(self.end_img_var.get() if hasattr(
                    self, 'end_img_var') else 'tansuojieshu.png', 0.7)
                while not end_pos and self.is_running:
                    self.pause_event.wait()

                    explore_pos = self.find_image(self.explore_img_var.get() if hasattr(
                        self, 'explore_img_var') else 'tansuo.png', 0.8)
                    if explore_pos is not None:
                        self.click_position(explore_pos)

                        # 战斗胜利处理
                        victory_pos = self.find_image(
                            self.victory_img_var.get(), 0.8)
                        if victory_pos:
                            self.click_position(victory_pos)

                        # 结算处理
                        reward_pos = self.find_image(
                            self.reward_img_var.get(), 0.8)
                        if reward_pos:
                            self.click_position(reward_pos)
                            time.sleep(1.5)

                        end_pos = self.find_image(self.end_img_var.get() if hasattr(
                            self, 'end_img_var') else 'tansuojieshu.png', 0.8)
                    else:
                        time.sleep(1)
                        end_pos = self.find_image(self.end_img_var.get() if hasattr(
                            self, 'end_img_var') else 'tansuojieshu.png', 0.8)
                ...
                if end_pos:
                    ...
                    if reward_pos:
                        ...
                        # 处理战利品
                        treasure_pos = self.find_image(self.treasure_img_var.get() if hasattr(
                            self, 'treasure_img_var') else 'zhanlipin.png', 0.8)
    ...
    def jiejietupo_strategy(self):
        ...
        try:
            ...
            for i in range(1, cycles + 1):
                ...
                # 查找突破按钮
                start_pos = self.find_image(self.battle_img_var.get(), 0.8)
                if not start_pos:
                    self.log("未找到突破按钮")
                    if error_action == "stop":
                        break
                ...
                attack_img = self.attack_img_var.get() if hasattr(
                    self, 'attack_img_var') else 'jingongjiejie.png'
                attack_pos = self.find_image(attack_img, 0.8)
                if not attack_pos:
                    self.log("未找到攻击按钮")
                    if error_action == "stop":
                        break
                ...
                # 查找结算或失败图片
                reward_pos = self.find_image(self.reward_img_var.get(), 0.8)
                defeat_pos = self.find_image(self.defeat_img_var.get(), 0.8)

                result_pos = reward_pos or defeat_pos
                if result_pos:
                    ...
                    # 检查是否还有额外结算
                    another_pos = self.find_image(
                        self.reward_img_var.get(), 0.7)
                    if another_pos:
                        self.click_position(another_pos)
    ...
    def default_strategy(self):
        ...
        try:
            ...
            for i in range(1, cycles + 1):
                ...
                # 查找并点击战斗开始按钮
                battle_pos = self.find_image(self.battle_img_var.get(), 0.8)
                if not battle_pos:
                    self.log("未找到战斗开始按钮")
                    if error_action == "stop":
                        break
                ...
                # 查找胜利或失败界面
                victory_pos = self.find_image(self.victory_img_var.get(), 0.8)
                defeat_pos = None

                if not victory_pos:
                    defeat_pos = self.find_image(
                        self.defeat_img_var.get(), 0.8)

                # 点击结果界面
                result_pos = victory_pos or defeat_pos
                if result_pos:
