...
class YYSBotGUI:
    ...
    def find_and_click(self, image_path, confidence=0.8, clicks=None, click_when_not_found=True):
        try:
        ...
        try:
            ...
            for i in range(1, cycles + 1):
                ...
                if not victory_found and not defeat_found:
                    self.log("未检测到战斗结果界面，已执行随机点击")

                # 点击结算界面 (找不到也会点击)
                time.sleep(1)
                reward_found = self.find_and_click(
                    self.reward_img_var.get(), confidence)

                if not reward_found:
                    self.log("未找到战利品结算界面，已执行随机点击")
