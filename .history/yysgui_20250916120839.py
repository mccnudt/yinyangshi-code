...
class YYSBotGUI:
    ...
    def setup_ui(self):
        ...
        self.rest_cycles_var = tk.StringVar(
            value=str(self.current_template["rest_cycles"]))
        ttk.Entry(param_frame, textvariable=self.rest_cycles_var, width=5).grid(
            row=3, column=1, padx=(5, 0), sticky=tk.W, pady=(5, 0))
        ttk.Label(param_frame, text="次战斗后休息").grid(
            row=3, column=1, sticky=tk.W, padx=(40, 0), pady=(5, 0))
        ...
        ttk.Label(param_frame, text="秒(如5-10)").grid(row=3,
    ...
    def load_template(self, event=None):
        ...
        try:
            ...
            if template:
                ...
                self.rest_cycles_var.set(template.get("rest_cycles", "10"))
    ...
    def save_template(self):
        ...
        template = {
            ...
            "rest_cycles": int(self.rest_cycles_var.get()),
    ...
    def get_random_rest_time(self, rest_time_str):
        try:
            parts = rest_time_str.split("-")
            if len(parts) == 2:
                min_val = int(parts[0])
                max_val = int(parts[1])
                return random.randint(min_val, max_val)
    ...
    def find_and_click(self, image_path, confidence=0.8, clicks=None):
        try:
    ...
    def battle_loop(self):
        try:
            ...
            for i in range(1, cycles + 1):
                ...
                # 查找并点击战斗开始按钮
                if not self.find_and_click(self.battle_img_var.get(), confidence):
                    self.log("未找到战斗开始按钮")
                ...
                victory_found = self.find_and_click(
                    self.victory_img_var.get(), confidence)
                defeat_found = False

                if not victory_found:
                    defeat_found = self.find_and_click(
                        self.defeat_img_var.get(), confidence)
                ...
                # 点击结算界面
                time.sleep(1)
                self.find_and_click(self.reward_img_var.get(), confidence)

                # 检查是否需要休息
                if i % rest_cycles == 0 and i < cycles:
                    rest_time = self.get_random_rest_time(
