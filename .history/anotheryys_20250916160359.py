...
class YYSBotGUI:
    ...
    def find_image(self, image_path, confidence=0.8, timeout=1):
        """查找图片位置，带超时"""
        try:
    ...
    def default_strategy(self):
        ...
        try:
            ...
            for i in range(1, cycles + 1):
                ...
                while not result_found and (time.time() - start_time) < max_wait_time and self.is_running:
                    self.pause_event.wait()
            ...
            try:
                ...
                for i in range(1, cycles + 1):
                    ...
                    # 优化后的检测逻辑：增加检测频率，减少总等待时间
                    max_wait_time = 15  # 最多等待15秒（可根据需要调整）
                    start_time = time.time()

                    while not result_pos and (time.time() - start_time) < max_wait_time and self.is_running:
                        self.pause_event.wait()
